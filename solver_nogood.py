...
def get_pc_conditional_sampler(controller, sde_x, sde_adj, shape_x, shape_adj, predictor='Euler', corrector='None', 
  ...
  def pc_sampler(model_x, model_adj, init_flags, w):
    ...
    with torch.no_grad():
      ...
      # gt_keep_mask_adj_[9, 17] = 0
      # gt_keep_mask_adj_[17, 9] = 0
      
      # gt_keep_mask_adj_[1, 6] = 1
      # gt_keep_mask_adj_[6, 1] = 1   
      
      # import pdb;pdb.set_trace()
      gt_keep_mask_adj_ = gt_keep_mask_adj_.expand(shape_x[0], -1, -1)
      # .expand(100, -1, -1)
      # # 生成mask的邻接矩阵
      ...
      betas = torch.linspace(sde_adj.T, eps, diff_steps, device=device)
      alphas = 1 - betas
      alpha_bars = torch.tensor([torch.prod(alphas[:ii + 1]) for ii in range(len(alphas))]).to(device)
      
      # -------- Reverse diffusion process --------
      for i in trange(0, (diff_steps), desc = '[Sampling]', position = 1, leave=False):
        t = timesteps[i]
        vec_t_x = torch.ones(shape_x, device=t.device) * t
        vec_t_adj = torch.ones(shape_adj, device=t.device) * t
        vec_t = torch.ones(shape_adj[0], device=t.device) * t
        # import pdb;pdb.set_trace()
        # if i>1 and i%10==0:
        ##### mask
        ...
        if i>0:
          ...
          # import pdb;pdb.set_trace()
          gt_weight_x = torch.sqrt(vec_t_x)
          # torch.sqrt(vec_t_x)
          ...
          # import pdb;pdb.set_trace()
          # noise_weight_x = 1 - alpha_cumprod_x
          # noise_weight_x = alpha_cumprod_x
          # noise_part_x = noise_weight_x * torch.randn_like(x)
          # weighed_gt_x = gt_part_x
          # weighed_gt_x = gt_part_x * gt_weight_x
          ...
          gt_weight_adj = torch.sqrt(vec_t_adj)
          ...
          # gt_part_adj = gt_adj
          # noise_weight_adj = 1 - alpha_cumprod_adj
          # noise_weight_adj = alpha_cumprod_adj
          # noise_part_adj = noise_weight_adj * torch.randn_like(adj)

          # weighed_gt_adj = gt_part_adj
          # weighed_gt_adj = gt_part_adj * gt_weight_adj
          
          # import pdb;pdb.set_trace()
          # x_un_t_1 = 1/torch.sqrt(alphas[i]) * ( x- betas[i]/(torch.sqrt(1-(alpha_bars[i]))) * (score_fn_x(x, adj, condition[0], flags, vec_t)*(1-0.9)+score_fn_x(x, adj, condition[1], flags, vec_t)*(0.9)) ) + torch.randn_like(x) * t
          # adj_un_t_1 = 1/torch.sqrt(alphas[i]) * ( adj- betas[i]/(torch.sqrt(1-(alpha_bars[i])))  * (score_fn_adj(x, adj, condition[0], flags, vec_t)*(1-0.9)+score_fn_adj(x, adj, condition[1], flags, vec_t)*(0.9)) ) + torch.randn_like(adj) * t
          
          
        
          # adj =  (1 - gt_keep_mask_adj_ ) * ( weighed_gt_adj ) + gt_keep_mask_adj_ * adj_un_t_1  
          # x =  (1 - gt_keep_mask_x_ ) * ( weighed_gt_x ) + gt_keep_mask_x_ * x_un_t_1 
          adj =  (1 - gt_keep_mask_adj_ ) * ( gt_part_adj ) +  adj *  (torch.sqrt(1-vec_t_adj))
          x =  (1 - gt_keep_mask_x_ ) * ( gt_part_x ) +   x * (torch.sqrt(1-vec_t_x))
          # print(x_un_t_1.sum().item(), adj_un_t_1.sum().item())
          print('\n')
          print(x.sum().item(), adj.sum().item(), x_mean.sum().item(), adj_mean.sum().item())
          print(gt_x.sum().item(), x.sum().item())
          print(gt_adj.sum().item(), adj.sum().item())
        ...
        #   #  and i%==0
        #   print(x.sum().item(), adj.sum().item(), x_mean.sum().item(), adj_mean.sum().item())
        #   print(gt_part_x.sum().item(), x.sum().item())
        #   print(gt_part_adj.sum().item(), adj.sum().item())
      # print(f'w is {w}.')
      ...
      # import pdb;pdb.set_trace()
      max_values, max_indices = torch.max(x_mean, dim=2)
      mask = (x_mean == max_values.unsqueeze(-1)) & (x_mean > 0.5)
      x_result = mask.float()
      x_mean =  (1 - gt_keep_mask_x_) * gt_x  + gt_keep_mask_x_ * ( x_result )
      
      # gt_keep_mask_adj_[:, 9, 17] = 0.5
      # gt_keep_mask_adj_[:, 17, 9] = 0.5  
      
      # adj_mean =  (1 - gt_keep_mask_adj_) * gt_adj   +  ( adj_mean )
      ...
      # max_values, max_indices = torch.max(x, dim=2)
      # mask = (x == max_values.unsqueeze(-1)) & (x > 0.5)
      # x_result = mask.float()
      # x =  (1 - gt_keep_mask_x_) * gt_x  + gt_keep_mask_x_ * ( x_result )
      # # adj_mean =  (1 - gt_keep_mask_adj_) * gt_adj   + gt_keep_mask_adj_ * ( adj_mean ) / 10
      # # import pdb;pdb.set_trace()

      # gt_keep_mask_adj__ = torch.zeros(shape_x[1], shape_x[1], device=device)+0.5
      # # import pdb;pdb.set_trace()
      # # 将需要隐藏的节点及其所有边关系置为1
      # for i in range(len(gt_keep_mask_adj)):
      #     if gt_keep_mask_adj[i] == 1:
      #         gt_keep_mask_adj__[i, :] = 1
      #         gt_keep_mask_adj__[:, i] = 1
              
      
      # # import pdb;pdb.set_trace()
      # gt_keep_mask_adj__ = gt_keep_mask_adj_.expand(shape_x[0], -1, -1)
      
      # adj =  (1 - gt_keep_mask_adj_) * gt_adj * 0.5   + (gt_keep_mask_adj__) * ( adj )
      
      # return x, adj, diff_steps * (n_steps + 1), gt_x, gt_adj, gt_keep_mask_x_, gt_keep_mask_adj_
      ...
      # import pdb;pdb.set_trace()
      # x_mean =  (1 - gt_keep_mask_x_) * ( gt_x  )   
      # # + gt_keep_mask_x_ * ( x ) 
      # adj_mean =  (1 - gt_keep_mask_adj_) * ( gt_adj ) 
        # + gt_keep_mask_adj_ * ( adj ) 
