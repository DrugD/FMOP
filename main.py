...
from utils.mol_utils import gen_mol, mols_to_smiles, load_smiles, canonicalize_smiles, mols_to_nx, filter_smiles_with_labels, correct_benzene_rings
...
def drug_cell_response_regression_generation(model_type = 'molgen', condition_strength=1.0, seed=42, gen_number=1000, timestep = 1000, ic50 = 0.4, cell_line = '1290730', device='cpu', internal = 0):
    ...
    if model_type == 'molgen':
        ...
        return sampler.sample(train_smiles=train_smiles)
    elif model_type == 'multimolgen':
        
        # 加载保存的 JSON 文件
        with open('/home/lk/project/repaint/MolPaint/log/ic50_result_dict_20_30.json', 'r', encoding='utf-8') as f:
        ...
        internal = internal
        index_start = internal*50
        index_end = (internal+1)*50 if (internal+1)*50 < len(ic50_result_dict.items()) else len(ic50_result_dict.items())
        
        dddd = []
        for index, (cell_line, drugs) in tqdm(enumerate(ic50_result_dict.items())):
            # if cell_line != '1524415':
            #     continue
            # import pdb;pdb.set_trace()
            # if start_flag == False:
            #     continue
            if index_start <= index and  index < index_end:
...
if __name__ == '__main__':
    ...
    # 显存需要至少1G/100 +470
    generation_results = drug_cell_response_regression_generation(model_type=model_type, condition_strength=5.0, seed=42, gen_number=100, timestep = 300, ic50 = None, cell_line = None, device=device, internal = internal)
