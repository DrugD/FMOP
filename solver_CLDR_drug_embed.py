...
def get_pc_conditional_sampler(controller, sde_x, sde_adj, shape_x, shape_adj, predictor='Euler', corrector='None', 
  ...
  drp_label_embedding = TransEDRP(config_controller, params_x, params_adj, device, config.train)
  drp_label_embedding.load_state_dict(torch.load(config_controller.cldr_ckpt, map_location=device)['model_state_dict'], strict = True)
  drp_label_embedding.to(device)
  drp_label_embedding.eval()
  ...
  def pc_sampler(model_x, model_adj, init_flags, w):
    ...
    def conditional_corrector_update_fn_x(score_fn_x, x, adj, flags, vec_t, gts):
      corrector_fn = LangevinCorrector if corrector=='Langevin' else NoneCorrector
    ...
    def conditional_corrector_update_fn_adj(score_fn_adj, x, adj, flags, vec_t, gts):
      corrector_fn = LangevinCorrector if corrector=='Langevin' else NoneCorrector
    ...
    def conditional_predictor_update_fn_x(score_fn_x, x, adj, flags, vec_t, gts):
      predictor_fn = ReverseDiffusionPredictor if predictor=='Reverse' else EulerMaruyamaPredictor 
      
      # gen_drug_embed = drp_label_embedding.get_drug_embed(x, adj, None, False)
      # gt_drug_embed = drp_label_embedding.get_drug_embed(gt_x, gt_adj, None, False)
    ...
    def conditional_predictor_update_fn_adj(score_fn_adj, x, adj, flags, vec_t, gts):
      predictor_fn = ReverseDiffusionPredictor if predictor=='Reverse' else EulerMaruyamaPredictor 
    ...
    # with torch.no_grad():
      # -------- Initial sample --------
    x = sde_x.prior_sampling(shape_x).to(device).requires_grad_()
    adj = sde_adj.prior_sampling_sym(shape_adj).to(device).requires_grad_()
    ...
    gt_x = gt_x.expand(shape_x[0], -1, -1).to(device).requires_grad_()
    gt_adj =  torch.from_numpy(gt_adj).expand(shape_x[0], -1, -1).to(device).requires_grad_()
    ...
    gt_keep_mask_adj_[4, 5] = 0
    gt_keep_mask_adj_[5, 4] = 0    
    gt_keep_mask_adj_[6, 24] = 0
    gt_keep_mask_adj_[24, 6] = 0  
    ...
    # diff_steps = 100
    print(f'Diff_steps is {diff_steps}.')
    # torch.linspace(beta_start, beta_end, timesteps)
    timesteps = torch.linspace(sde_adj.T, eps, diff_steps, device=device)
    # timesteps = torch.linspace(eps, 0.05, diff_steps, device=device)
    # alpha = 1-beta
    ...
    with torch.no_grad():
      # 定义用于优化显存利用的函数
      def optimized_embedding_calculation(gt_keep_mask_adj_, gt_keep_mask_x_, gt_adj, gt_x, drp_label_embedding, x, adj):
        gts_ = (drp_label_embedding.get_drug_embed((1 - gt_keep_mask_x_) * x, (1 - gt_keep_mask_adj_) * adj, None, False) -
                  drp_label_embedding.get_drug_embed((1 - gt_keep_mask_x_) * gt_x, (1 - gt_keep_mask_adj_) * gt_adj, None, False)).norm()
      ...
      for i in trange(0, (diff_steps), desc = '[Sampling]', position = 1, leave=False):
        t = timesteps[i]
        vec_t = torch.ones(shape_adj[0], device=t.device) * t
        # alpha_bar_x = torch.ones(shape_x, device=t.device) * alpha_bar[i]
        # alpha_bar_adj = torch.ones(shape_adj, device=t.device) * alpha_bar[i]
        
        # alpha_x = torch.ones(shape_x, device=t.device) * alpha[i]
        # alpha_adj = torch.ones(shape_adj, device=t.device) * alpha[i]
        

        gts_ = optimized_embedding_calculation(gt_keep_mask_adj_, gt_keep_mask_x_, gt_adj, gt_x, drp_label_embedding, x, adj)

        _x = x
        x, x_mean = conditional_corrector_update_fn_x(score_fn_x, x, adj, flags, vec_t, gts_)
        adj, adj_mean = conditional_corrector_update_fn_adj(score_fn_adj, _x, adj, flags, vec_t, gts_)
        
        _x = x
        x, x_mean = conditional_predictor_update_fn_x(score_fn_x, x, adj, flags, vec_t, gts_)
        adj, adj_mean = conditional_predictor_update_fn_adj(score_fn_adj, _x, adj, flags, vec_t, gts_)
  ...
  # 
        ...
        # difference_adj = (1 - gt_keep_mask_adj_) * ( gt_adj ) - (1 - gt_keep_mask_adj_) * zero_adj
        # norm_adj = torch.linalg.norm(difference_adj)
        
        # import pdb;pdb.set_trace()
        # norm_grad_adj = torch.autograd.grad(outputs=norm_adj, inputs=adj)[0]
        
        # difference_x = (1 - gt_keep_mask_x_) * ( gt_x ) - (1 - gt_keep_mask_x_) * zero_x
        # norm_x = torch.linalg.norm(difference_x)
        # norm_grad_x = torch.autograd.grad(outputs=norm_x, inputs=x)[0]
        ...
        # score_fn_adj(x, adj, condition[0], flags, t)*(1-w)+score_fn_adj(x, adj, condition[1], flags, t)*(w)
        # score_fn_x(x, adj, condition[0], flags, t)*(1-w)+score_fn_x(x, adj, condition[1], flags, t)*(w)
        
        # import pdb;pdb.set_trace()

        # _x = x
        # zero_x = 1/torch.sqrt(alpha_bar_x) * (x+(1-alpha_bar_x)*(score_fn_x(x, adj, condition[0], flags, vec_t)*(1-w)+score_fn_x(x, adj, condition[1], flags, vec_t)*(w)))
        # zero_adj = 1/torch.sqrt(alpha_bar_adj) * (adj+(1-alpha_bar_adj)*(score_fn_adj(x, adj, condition[0], flags, vec_t)*(1-w)+score_fn_adj(x, adj, condition[1], flags, vec_t)*(w)))

        # # Calculate differences and gradients
        # difference_adj = (1 - gt_keep_mask_adj_) * gt_adj - (1 - gt_keep_mask_adj_) * zero_adj
        # norm_adj = torch.linalg.norm(difference_adj)
        # norm_grad_adj = torch.autograd.grad(outputs=norm_adj, inputs=adj, allow_unused=True)[0]

        # difference_x = (1 - gt_keep_mask_x_) * gt_x - (1 - gt_keep_mask_x_) * zero_x
        # norm_x = torch.linalg.norm(difference_x)
        # norm_grad_x = torch.autograd.grad(outputs=norm_x, inputs=x, allow_unused=True)[0]

        # 保存梯度
        # adj_grad = adj.grad.clone().detach()
        # x_grad = x.grad.clone().detach()
        # adj = adj-norm_grad_adj
        # x = x-norm_grad_x
        ...
        # if i%10==0:  
        print(f"{i}")
        # Printing sums for debugging
        print(f'x is {round(x.sum().item(), 3)}, adj is {round(adj.sum().item(), 3)}, x_mean is {round(x.mean().item(), 3)}, adj_mean is {round(adj.mean().item(), 3)}')
        print(f'gt_x sum is {round(gt_x.sum().item(), 3)}, x sum is {round(x.sum().item(), 3)}')
        print(f'gt_adj sum is {round(gt_adj.sum().item(), 3)}, adj sum is {round(adj.sum().item(), 3)}')
        ...
        # del norm_grad_adj, norm_grad_x, difference_adj, difference_x, zero_x, zero_adj
        # torch.cuda.empty_cache()
        
      return (x_mean if denoise else x), (adj_mean if denoise else adj), diff_steps * (n_steps + 1),  gt_x, gt_adj, gt_keep_mask_x_, gt_keep_mask_adj_
  return pc_sampler
