...
def get_pc_conditional_sampler(controller, sde_x, sde_adj, shape_x, shape_adj, predictor='Euler', corrector='None', 
  ...
  def pc_sampler(model_x, model_adj, init_flags, w):
    ...
    with torch.no_grad():
      ...
      # # 保留接口 遍历mask_x中值为1的节点
      # for i in range(shape_adj[1]):
      #     for j in range(shape_adj[2]):
      #       if gt_adj[0][i, j].item() == 1 and gt_keep_mask_x[i].item() == 1:
      #         # 获取邻居节点
      #         neighbors = gt_adj[0][i].nonzero(as_tuple=True)[0]
      #         for neighbor in neighbors:
      #             if gt_keep_mask_x[neighbor] == 0:
      #                 print(f'{i},{neighbor} is remain 0 for linker.')
      #                 gt_keep_mask_adj_[i, neighbor] = 0
      #                 gt_keep_mask_adj_[neighbor, i] = 0
      #         neighbor_mask = gt_keep_mask_x[neighbors]
                
      #         # 检查邻居是否在mask中，并且部分为0部分为1
      #         if (neighbor_mask == 0).any() and (neighbor_mask == 1).any():
      #             # 将gt_keep_mask_adj_中对应的行列值赋值为1
      #             print(f'{i},{j} is remain 0 for linker.')
      #             import pdb;pdb.set_trace()
      #             gt_keep_mask_adj_[i, j] = 0
      #             gt_keep_mask_adj_[j, i] = 0
                    
      # gt_keep_mask_adj_ = gt_keep_mask_adj_.unsqueeze(0).expand(shape_adj[0], -1, -1)
      # .expand(100, -1, -1)
      # # 生成mask的邻接矩阵
