...
def train(model, transe, dataloader, optimizer, criterion, device):
    ...
    for index, data in tqdm(enumerate(dataloader)):
        ...
        loss_t = cross_entropy_loss_1(logits_per_text, labels)
        loss_dc_num = cross_entropy_loss_1(num_logits_per_dc, labels)
        loss_t_num = cross_entropy_loss_1(num_logits_per_text, labels)
        
        loss_CLIP = (loss_dc + loss_t)/2 
        
        loss_CLIP_Num = (loss_dc_num + loss_t_num)/2 
        
        loss = loss_TransE*0.0001 + loss_CLIP * 0.8 + loss_CLIP_Num * 0.2
        loss = loss_CLIP
        ...
        total_loss[0] += loss
        total_loss[1] += loss_TransE
        total_loss[2] += loss_CLIP
        total_loss[3] += loss_CLIP_Num
        
        logger.info(f"Iter[{index+1}], Step train loss: {total_loss[0].item()/(index+1)}, TransE loss: {total_loss[1].item()/(index+1)}, CLIP loss: {total_loss[2].item()/(index+1)}, CLIP Num loss: {total_loss[3].item()/(index+1)}")
       
        # 手动释放不再需要的变量
        del number_features, labels
...
def evaluate(model, transe, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for index, data in tqdm(enumerate(dataloader)):
            ...
            logger.info(f"Iter[{index+1}], Step valid loss: {total_loss[0].item()/(index+1)}, TransE loss: {total_loss[1].item()/(index+1)}, CLIP loss: {total_loss[2].item()/(index+1)}, CLIP Num loss: {total_loss[3].item()/(index+1)}")
...
for epoch in tqdm(range(num_epochs)):
    ...
    logger.info(f"Epoch[{epoch}] train loss: {train_loss[0].item()}, TransE loss: {train_loss[1].item()}, CLIP loss: {train_loss[2].item()}, CLIP Num loss: {train_loss[3].item()}")

    val_loss = evaluate(model, transe, val_loader, criterion, device)
    logger.info(f"Epoch[{epoch}] valid loss: {val_loss[0].item()}, TransE loss: {val_loss[1].item()}, CLIP loss: {val_loss[2].item()}, CLIP Num loss: {val_loss[3].item()}")

    # 检查是否有新的最优验证损失
    if val_loss[0].item() < best_val_loss:
        best_val_loss = val_loss[0].item()
        best_epoch = epoch + 1
        epochs_no_improve = 0
